...
import {
  ...
  SignedIntentProof,
  FeePaymentMethod,
} from './generated/ottochain/v1/delegation.js';
 ...
 */
export function createDelegation(config: DelegationConfig): DelegationAuthority {
  ...
  const fullScope: DelegationScope = {
 ...
 */
export function isDelegationValid(delegation: DelegationAuthority): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
 ...
 */
export function createRelayedTransaction(
  transaction: Record<string, unknown>,
  delegationProof: { type: 'sessionKey'; proof: SessionKeyProof } | { type: 'signedIntent'; proof: SignedIntentProof },
  gasConfig: GasConfig,
...
export async function submitDelegated(
  ...
  delegation: DelegationAuthority,
  bridgeUrl?: string
): Promise<{ txId: string; status: string; receipt?: unknown }> {
  ...
  let delegationProof: { type: 'sessionKey'; proof: SessionKeyProof } | { type: 'signedIntent'; proof: SignedIntentProof };
  ...
  if (delegation.approach === DelegationApproach.DELEGATION_APPROACH_SESSION_KEY) {
    ...
    const sessionKeyProof: SessionKeyProof = {
      sessionKey: {
    ...
    const signedIntentProof: SignedIntentProof = {
      signedIntent: {
        delegationId: delegation.delegationId,
        transaction: Struct.fromJSON(transaction),
  ...
  const relayedTx = createRelayedTransaction(
    transaction,
    delegationProof,
    gasConfig,
...
export async function listDelegations(
  ...
  if (options.includeExpired) queryParams.set('includeExpired', 'true');
  if (options.includeRevoked) queryParams.set('includeRevoked', 'true');

  try {
    const response = await fetch(
      `${url}/api/delegations?principal=${encodeURIComponent(principalAddress)}&${queryParams.toString()}`,
      {
        method: 'GET',
        headers: {
          'Accept': 'application/json',
 ...
 */
export function timeWindow(start: Date, end: Date): Partial<DelegationScope> {
  return {
 ...
 */
export function actionFilter(allowedActions: string[]): Partial<DelegationScope> {
  return {
 ...
 */
export function amountLimit(maxAmount: number): Partial<DelegationScope> {
  return {
    policyRules: Value.fromJSON({
 ...
 */
export function combineScopes(...scopes: Partial<DelegationScope>[]): DelegationScope {
  const combined: DelegationScope = {
    fiberIds: [],
    allowedOperations: [],
  };
  ...
  if (policyRules.length > 0) {
    combined.policyRules = Value.fromJSON(
      policyRules.length === 1 ? policyRules[0] : { and: policyRules }
    );
  }
 ...
 */
function createDelegationSignaturePayload(delegation: DelegationAuthority): string {
 ...
 */
function createRevocationSignaturePayload(revocation: DelegationRevocation): string {
