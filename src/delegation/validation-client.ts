...
export interface ValidationError {
  type: 'SESSION_KEY_EXPIRED' | 'DELEGATION_REVOKED' | 'SCOPE_VIOLATION' | 
        'SPENDING_LIMIT_EXCEEDED' | 'REPUTATION_TOO_LOW' | 'SIGNATURE_INVALID';
  message: string;
  details?: any;
}
...
export class DelegationValidationClient {
  private config: DelegationValidationConfig;
  private cache = new Map<string, { data: DelegationInfo; expiresAt: number }>();
   ...
   */
  async validateOperation(request: ValidationRequest): Promise<ValidationResponse> {
   ...
   */
  async getDelegationInfo(delegationId: string): Promise<DelegationInfo | null> {
    // Check cache first
   ...
   */
  private validateScope(request: ValidationRequest, scope: DelegationScope): ValidationError[] {
    const errors: ValidationError[] = [];
   ...
   */
  private async performRemoteValidation(request: ValidationRequest): Promise<ValidationError[]> {
    try {
   ...
   */
  private mapRemoteErrorType(remoteType?: string): ValidationError['type'] {
    const mapping: Record<string, ValidationError['type']> = {
 ...
 */
export function createDelegationValidationClient(
  bridgeEndpoint: string,
  options?: Partial<DelegationValidationConfig>
): DelegationValidationClient {
...
export function isValidationError(response: ValidationResponse): response is ValidationResponse & { errors: ValidationError[] } {
  return !response.isValid && !!response.errors;
}