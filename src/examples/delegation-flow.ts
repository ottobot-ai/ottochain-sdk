...
export async function createBasicSessionKey() {
  ...
  try {
    ...
    const response = await client.createSessionKey(
      USER_ADDRESS,
      {
...
export async function createMarketOperationsDelegation() {
  ...
  try {
    ...
    const scope = DelegationHelpers.createMarketScope(
      '50.0',  // Max per transaction: 50 DAG
...
export async function createAndSignIntent() {
  ...
  try {
    ...
    const delegationResponse = await client.createSessionKey(
      USER_ADDRESS,
      {
        delegateAddress: AGENT_ADDRESS,
        scope,
    ...
    const intentResponse = await client.signIntent(
      USER_ADDRESS,
      {
        delegationId,
        intent,
...
export async function manageDelegations() {
  ...
  try {
    ...
    if (activeDelegations.length > 0) {
      ...
      const revocationResponse = await client.revokeDelegation(
        toRevoke.delegationId,
        USER_ADDRESS,
        'Testing revocation in example',
        'revocation_signature_placeholder',
        10 // nonce
      );
...
export async function advancedDelegationScopes() {
  ...
  try {
    ...
    const customScope: DelegationScope = {
      allowedOperations: [
...
export async function errorHandlingExample() {
  ...
  try {
    ...
    if (!validation.isValid) {
      ...
      throw new DelegationError(
        ...
        'SCOPE_VIOLATION',
        { scope: invalidScope, errors: validation.errors }
      );
    }
  ...
  try {
    ...
    console.log('Expected timeout or network error:', 
...
export async function batchOperationsExample() {
  ...
  try {
    ...
    const batchResults = await client.batchRevokeDelegations(
      delegationIds,
      USER_ADDRESS,
      'Batch revocation test',
      'batch_revocation_signature',
      20 // starting nonce
    );
