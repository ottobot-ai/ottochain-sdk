...
export const corpShareholdersDef = defineFiberApp({
  ...
  stateSchema: {
    properties: {
      ...
      recordDate: { $ref: '#/definitions/RecordDate', nullable: true, computed: true },
      eligibleVoters: { type: 'array', items: { $ref: '#/definitions/EligibleVoter' }, computed: true },
      quorumRequirements: { $ref: '#/definitions/QuorumRequirements', computed: true },
      agenda: { type: 'array', items: { $ref: '#/definitions/AgendaItem' }, computed: true },
      proxyPeriod: { $ref: '#/definitions/ProxyPeriod', nullable: true, computed: true },
      votes: { type: 'array', items: { $ref: '#/definitions/Vote' }, computed: true },
      voteTallies: { type: 'array', items: { $ref: '#/definitions/VoteTally' }, computed: true },
      inspectorOfElections: { $ref: '#/definitions/Inspector', nullable: true, computed: true },
      sessionInfo: { $ref: '#/definitions/SessionInfo', nullable: true, computed: true },
      certification: { $ref: '#/definitions/Certification', nullable: true, computed: true },
      status: {
        type: 'string',
        enum: ['SCHEDULED', 'RECORD_DATE_SET', 'PROXY_PERIOD', 'IN_SESSION', 'VOTING', 'CLOSED'] as const,
        computed: true,
      },
  ...
  eventSchemas: {
    schedule_annual: {
      ...
      required: ['meetingId', 'entityId', 'fiscalYear', 'scheduledDate', 'boardResolutionRef'] as const,
      properties: {
    ...
    schedule_special: {
      ...
      required: ['meetingId', 'entityId', 'scheduledDate', 'purpose', 'calledByType'] as const,
      properties: {
        meetingId: { type: 'string' },
        entityId: { type: 'string' },
        scheduledDate: { type: 'string', format: 'date-time' },
        location: { type: 'object' },
        purpose: { type: 'string', description: 'Specific purpose for special meeting' },
        calledByType: { type: 'string', enum: ['BOARD', 'SHAREHOLDERS', 'COURT'] as const },
    ...
    open_proxy_period: {
      ...
      required: ['startDate', 'proxyStatementRef', 'formOfProxyRef', 'agenda'] as const,
      properties: {
    ...
    open_meeting: {
      ...
      required: ['openedAt', 'chairPerson', 'secretaryPresent', 'initialQuorumCount'] as const,
      properties: {
    ...
    cast_vote: {
      ...
      required: ['voteId', 'agendaItemId', 'voterId', 'shareholderId', 'shareClass'] as const,
      properties: {
        voteId: { type: 'string' },
      ...
      properties: {
        ...
        results: {
          ...
          items: {
            ...
            properties: {
              agendaItemId: { type: 'string' },
              result: { type: 'string', enum: ['APPROVED', 'REJECTED'] as const },
            },
          },
        },
  ...
  definitions: {
    CalledBy: {
      type: 'object',
      properties: {
        type: { type: 'string', enum: ['BOARD', 'SHAREHOLDERS', 'COURT'] as const },
    ...
    NoticeInfo: {
      ...
      properties: {
        ...
        noticeMethod: { type: 'string', enum: ['MAIL', 'EMAIL', 'ELECTRONIC_ACCESS'] as const },
    ...
    QuorumRequirements: {
      type: 'object',
      properties: {
        type: { type: 'string', enum: ['SHARES_REPRESENTED', 'SHARES_OUTSTANDING'] as const },
    ...
    AgendaItem: {
      ...
      properties: {
        ...
        type: {
          type: 'string',
          enum: ['DIRECTOR_ELECTION', 'AUDITOR_RATIFICATION', 'SAY_ON_PAY', 'CHARTER_AMENDMENT', 'MERGER', 'STOCK_PLAN', 'SHAREHOLDER_PROPOSAL', 'OTHER'] as const,
        },
        voteRequired: {
          type: 'string',
          enum: ['PLURALITY', 'MAJORITY_CAST', 'MAJORITY_OUTSTANDING', 'SUPERMAJORITY', 'UNANIMOUS'] as const,
        ...
        status: { type: 'string', enum: ['PENDING', 'VOTING', 'CLOSED', 'APPROVED', 'REJECTED'] as const },
      },
    },
    ProxyPeriod: {
      type: 'object',
    ...
    VoteTally: {
      ...
      properties: {
        ...
        result: { type: 'string', enum: ['APPROVED', 'REJECTED', 'PENDING'] as const },
  ...
  states: {
    SCHEDULED: { id: 'SCHEDULED', isFinal: false, description: 'Meeting has been scheduled but record date not yet set' },
    RECORD_DATE_SET: { id: 'RECORD_DATE_SET', isFinal: false, description: 'Record date established; eligible shareholders determined' },
    PROXY_PERIOD: { id: 'PROXY_PERIOD', isFinal: false, description: 'Proxy materials distributed; shareholders may submit proxies' },
    IN_SESSION: { id: 'IN_SESSION', isFinal: false, description: 'Meeting is convened and in progress' },
    VOTING: { id: 'VOTING', isFinal: false, description: 'Polls are open for voting on agenda items' },
    CLOSED: { id: 'CLOSED', isFinal: true, description: 'Meeting concluded; results certified' },
  },
          ...
          { var: 'state' },
          {
            ...
            quorumRequirements: {
              merge: [
                ...
                {
                  sharesRequired: { '*': [{ var: 'event.totalSharesOutstanding' }, 0.5] },
                },
              ],
            },
          },
                ...
                [
                  {
                    ...
                    allowCumulativeVoting: { var: 'event.allowCumulativeVoting' },
                    status: 'PENDING',
                  },
                ],
              ],
        ...
        and: [
          {
            some: [
              ...
              { '==': [{ var: '.shareholderId' }, { var: 'event.shareholderId' }] },
            ],
          },
          {
            '!': {
              some: [
                { var: 'state.votes' },
                {
                  and: [
                    { '==': [{ var: '.shareholderId' }, { var: 'event.shareholderId' }] },
                    { '==': [{ var: '.agendaItemId' }, { var: 'event.agendaItemId' }] },
                  ],
                },
              ],
            },
                ...
                [
                  {
                    ...
                    cumulativeVoteAllocation: { var: 'event.cumulativeVoteAllocation' },
  ...
  crossReferences: {
    ...
    proxies: { machine: 'corporate-proxy', description: 'Proxy grants for this meeting', foreignKey: 'meetingId' },
    resolutions: { machine: 'corporate-resolution', description: 'Resolutions resulting from meeting votes', foreignKey: 'meetingId' },
    securities: { machine: 'corporate-securities', description: 'Share records for determining voting rights', foreignKey: 'entityId' },
  },
});
