...
export const corpShareholdersDef = defineFiberApp({
  ...
  eventSchemas: {
    ...
    schedule_annual: {
      description: 'Phase 2 of annual-meeting scheduling: execute once the bound board resolution is EXECUTED',
      required: ['meetingId', 'entityId', 'fiscalYear', 'scheduledDate'] as const,
      properties: {
        meetingId: { type: 'string' },
        entityId: { type: 'string' },
        fiscalYear: { type: 'integer' },
  ...
  definitions: {
    ...
    EligibleVoter: {
      ...
      properties: {
        ...
        address: {
          type: 'address',
          description: 'State-pinned DAG wallet address of record; cast_vote requires this address ∈ proofs[].address.',
        },
        shareholdings: {
          type: 'array',
          items: {
    ...
    AgendaItem: {
      ...
      properties: {
        ...
        status: {
          type: 'string',
          enum: ['PENDING', 'VOTING', 'CLOSED', 'APPROVED', 'REJECTED'] as const,
        },
      },
    },
    ProxyPeriod: {
  ...
  transitions: [
    ...
    // EXECUTED. depInState replaces the dropped object-form dependency (which silently never gated).
    {
      ...
      guard: {
        and: [
          ...
          {
            '==': [{ var: 'state.pendingschedule_annual.meetingId' }, { var: 'event.meetingId' }],
          },
      ...
      effect: {
        merge: [
          ...
          {
            _emit: [{ name: 'SHAREHOLDER_MEETING_SCHEDULED', data: { var: 'event' }, destination: 'external' }],
          },
        ],
      },
      dependencies: [],
    ...
    // schedule_special -> SCHEDULED
    {
      ...
      effect: {
        merge: [
          ...
          {
            _emit: [{ name: 'SPECIAL_MEETING_SCHEDULED', data: { var: 'event' }, destination: 'external' }],
          },
        ],
      },
    },
    ...
    // SCHEDULED -> RECORD_DATE_SET
    {
      ...
      effect: {
        merge: [
          ...
          {
            _emit: [{ name: 'RECORD_DATE_SET', data: { var: 'event' }, destination: 'external' }],
          },
        ],
      },
    },
      ...
      effect: {
        merge: [
          { var: 'state' },
          {
            ...
            noticeInfo: {
              merge: [{ var: 'state.noticeInfo' }, { noticeSentDate: { var: 'event.startDate' } }],
            },
          },
          {
            _emit: [{ name: 'PROXY_PERIOD_OPENED', data: { var: 'event' }, destination: 'external' }],
          },
        ],
      },
    },
    ...
    // PROXY_PERIOD -> IN_SESSION
    {
      ...
      guard: {
        '>=': [{ var: 'event.initialQuorumCount' }, { var: 'state.quorumRequirements.sharesRequired' }],
      },
      effect: {
        merge: [
          { var: 'state' },
          ...
          {
            _emit: [{ name: 'SHAREHOLDER_MEETING_OPENED', data: { var: 'event' }, destination: 'external' }],
          },
        ],
      },
    },
      ...
      effect: {
        merge: [
          { var: 'state' },
          {
            status: 'VOTING',
            sessionInfo: {
              merge: [{ var: 'state.sessionInfo' }, { pollsOpenedAt: { var: 'event.pollsOpenedAt' } }],
            },
            agenda: {
              map: [
                { var: 'state.agenda' },
            ...
            some: [
              { var: 'state.eligibleVoters' },
              {
                and: [
                  ...
                  {
                    in: [{ var: 'address' }, { map: [{ var: 'proofs' }, { var: 'address' }] }],
                  },
                ],
              },
            ],
      ...
      effect: {
        merge: [
          { var: 'state' },
          {
            status: 'IN_SESSION',
            sessionInfo: {
              merge: [{ var: 'state.sessionInfo' }, { pollsClosedAt: { var: 'event.pollsClosedAt' } }],
            },
            agenda: {
              map: [
                { var: 'state.agenda' },
    ...
    // IN_SESSION -> CLOSED (certify_results)
    {
      ...
      effect: {
        merge: [
          ...
          {
            _emit: [{ name: 'MEETING_RESULTS_CERTIFIED', data: { var: 'event' }, destination: 'external' }],
          },
        ],
      },
    },
      ...
      effect: {
        merge: [
          { var: 'state' },
          {
            status: 'CLOSED',
            sessionInfo: {
              merge: [{ var: 'state.sessionInfo' }, { adjournedAt: { var: 'event.adjournedAt' } }],
            },
          },
          {
            _emit: [{ name: 'MEETING_ADJOURNED', data: { var: 'event' }, destination: 'external' }],
          },
        ],
      },
    },
