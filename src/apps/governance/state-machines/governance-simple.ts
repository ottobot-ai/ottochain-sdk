...
export const govSimpleDef = defineFiberApp({
  ...
  createSchema: {
    required: ['admins', 'passingThreshold', 'disputeQuorum', 'votingPeriodMs'] as const,
    properties: {
      admins: {
        type: 'array',
  ...
  eventSchemas: {
    ...
    finalize: {
      description: 'Finalize the voting period (for-count derived from recorded ballots)',
      properties: {},
    },
    file_dispute: {
    ...
    dissolve: {
      description: 'Dissolve the organization (requires every member to sign this op — verified unanimity)',
      properties: {},
    },
  },
  ...
  transitions: [
    ...
    // VOTING → VOTING: vote (member, no double-voting)
    {
      ...
      guard: {
        and: [actorHasEntry('state.members'), { '!': [{ has: [{ var: 'state.votes' }, { var: 'event.agent' }] }] }],
      },
      effect: {
        merge: [
          { var: 'state' },
        ...
        '>=': [
          {
            length: [
              {
                filter: [{ values: [{ var: 'state.votes' }] }, { '===': [{ var: 'vote' }, 'for'] }],
              },
            ],
          },
          {
            '*': [{ length: [{ keys: [{ var: 'state.members' }] }] }, { var: 'state.passingThreshold' }],
          },
        ],
      },
      effect: {
        merge: [
          ...
          {
            rules: {
              merge: [{ var: 'state.rules' }, { var: 'state.proposal.changes' }],
            },
            history: {
              cat: [
                { var: 'state.history' },
    ...
    // DISPUTE → DISPUTE: submit_evidence (plaintiff or defendant)
    {
      ...
      guard: signerIsAnyParty(['state.dispute.plaintiff', 'state.dispute.defendant']),
      effect: {
        merge: [
          { var: 'state' },
          {
    ...
    // DISPUTE → ACTIVE: resolve (quorum met)
    {
      ...
      guard: {
        '>=': [{ length: [{ keys: [{ var: 'state.votes' }] }] }, { var: 'state.disputeQuorum' }],
      },
      effect: {
        merge: [
          { var: 'state' },
