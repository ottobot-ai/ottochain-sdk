...
export const daoTokenDef = defineFiberApp({
  ...
  transitions: [
    // ACTIVE → VOTING: propose (enough tokens)
    {
      ...
      guard: {
        some: [
          ...
          {
            '>=': [{ get: [{ var: 'state.balances' }, { var: '' }] }, { var: 'state.proposalThreshold' }],
          },
        ],
      },
      effect: {
    ...
    // VOTING → VOTING: vote (token holder, no double-vote, within window)
    {
      ...
      guard: {
        and: [
          actorIsSigner(),
          {
            '>': [{ get: [{ var: 'state.balances' }, { var: 'event.agent' }] }, 0],
          },
          {
            '!': [
              {
    ...
    // ACTIVE → ACTIVE: delegate
    {
      ...
      effect: {
        merge: [
          ...
          {
            delegations: {
              set: [{ var: 'state.delegations' }, { var: 'event.agent' }, { var: 'event.delegateTo' }],
            },
          },
        ],
      },
    ...
    // ACTIVE → ACTIVE: undelegate
    {
      ...
      // delegation.
      guard: {
        and: [actorIsSigner(), { has: [{ var: 'state.delegations' }, { var: 'event.agent' }] }],
      },
      effect: {
        merge: [
          { var: 'state' },
