...
export const daoReputationDef = defineFiberApp({
  ...
  createSchema: {
    required: ['memberThreshold', 'voteThreshold', 'proposeThreshold', 'quorum', 'votingPeriodMs'] as const,
    properties: {
      memberThreshold: { type: 'number', description: 'Min reputation to join as a member' },
      voteThreshold: { type: 'number', description: 'Min reputation to vote' },
      proposeThreshold: { type: 'number', description: 'Min reputation to submit a proposal' },
      quorum: { type: 'number', description: 'Min total votes (count) to execute' },
  ...
  eventSchemas: {
    propose: {
      ...
      required: ['proposalId', 'title', 'description', 'actionType', 'payload', 'agentReputation'] as const,
      properties: {
        agent: { type: 'address' },
            ...
            votes: {
              if: [
                { '===': [{ var: 'event.vote' }, 'for'] },
                {
                  merge: [
                    { var: 'state.votes' },
                    { for: { cat: [{ var: 'state.votes.for' }, [{ var: 'event.agent' }]] } },
                  ],
                },
                { '===': [{ var: 'event.vote' }, 'against'] },
                {
                  merge: [
                    { var: 'state.votes' },
                    { against: { cat: [{ var: 'state.votes.against' }, [{ var: 'event.agent' }]] } },
                  ],
                },
                {
                  merge: [
                    { var: 'state.votes' },
                    { abstain: { cat: [{ var: 'state.votes.abstain' }, [{ var: 'event.agent' }]] } },
                  ],
                },
              ],
            },
      ...
      effect: {
        merge: [
          { var: 'state' },
          {
            ...
            memberJoinedAt: {
              setKey: [{ var: 'state.memberJoinedAt' }, { var: 'event.agent' }, { var: '$timestamp' }],
            },
          },
        ],
      },
