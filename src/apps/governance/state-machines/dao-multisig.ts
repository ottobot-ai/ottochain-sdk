...
export const daoMultisigDef = defineFiberApp({
  ...
  eventSchemas: {
    ...
    dissolve: {
      description: 'Dissolve the DAO (requires every signer to sign this op — verified unanimity)',
      properties: {},
    },
  },
  ...
  transitions: [
    ...
    // PENDING → PENDING: sign (signer, no double-sign, not yet at threshold)
    {
      ...
      guard: {
        and: [
          ...
          {
            '<': [{ length: [{ keys: [{ var: 'state.signatures' }] }] }, { var: 'state.threshold' }],
          },
        ],
      },
      effect: {
        merge: [
          ...
          {
            signatures: {
              set: [{ var: 'state.signatures' }, { var: 'event.agent' }, { var: '$ordinal' }],
            },
          },
        ],
      },
          ...
          { var: 'state' },
          {
            cancelledProposals: {
              cat: [
                ...
                [
                  {
                    merge: [{ var: 'state.proposal' }, { cancelledAt: { var: '$ordinal' } }],
                  },
                ],
              ],
            },
    ...
    // ACTIVE → PENDING: propose_remove_signer (signers > threshold)
    {
      ...
      guard: {
        and: [
          actorInSet('state.signers'),
          {
            '>': [{ length: [{ var: 'state.signers' }] }, { var: 'state.threshold' }],
          },
        ],
      },
      effect: {
    ...
    // ACTIVE → PENDING: propose_change_threshold (1 <= new <= signers count)
    {
      ...
      guard: {
        and: [
          ...
          {
            '<=': [{ var: 'event.newThreshold' }, { length: [{ var: 'state.signers' }] }],
          },
        ],
      },
      effect: {
    ...
    // ACTIVE → DISSOLVED: dissolve (unanimous)
    {
      ...
      effect: {
        merge: [{ var: 'state' }, { dissolvedAt: { var: '$ordinal' }, status: 'DISSOLVED' }],
      },
      dependencies: [],
    },
  ],
