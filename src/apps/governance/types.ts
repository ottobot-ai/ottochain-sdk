...
export type SingleOwnerEvent =
  | { eventName: 'execute'; agent: string; actionId: string; actionType: string; payload: unknown }
  | { eventName: 'transfer_ownership'; agent: string; newOwner: string }
  | { eventName: 'accept_ownership'; agent: string }
  | { eventName: 'cancel_transfer'; agent: string }
  | { eventName: 'dissolve'; agent: string };
...
export interface MultisigDAOState {
  ...
  signatures: Record<string, number>; // signer -> timestamp
...
export type MultisigEvent =
  | { eventName: 'propose'; agent: string; proposalId: string; actionType: string; payload: unknown }
  | { eventName: 'sign'; agent: string }
  | { eventName: 'execute'; agent: string }
  | { eventName: 'cancel'; agent: string }
  | { eventName: 'propose_add_signer'; agent: string; proposalId: string; newSigner: string }
  | { eventName: 'propose_remove_signer'; agent: string; proposalId: string; removeSigner: string }
  | { eventName: 'propose_change_threshold'; agent: string; proposalId: string; newThreshold: number }
  | { eventName: 'apply_signer_change'; agent: string }
  | { eventName: 'dissolve'; agent: string; signatureCount: number };
...
export interface TokenDAOState {
  ...
  name: string;
  tokenId: string;
  balances: Record<string, number>; // address -> balance
  delegations: Record<string, string>; // delegator -> delegatee
...
export type TokenEvent =
  | { eventName: 'propose'; agent: string; proposalId: string; title: string; description: string; actionType: string; payload: unknown; snapshotBlock?: number }
  | { eventName: 'vote'; agent: string; vote: VoteChoice }
  | { eventName: 'queue'; agent: string }
  | { eventName: 'execute'; agent: string }
  | { eventName: 'reject'; agent: string }
  | { eventName: 'cancel'; agent: string }
  | { eventName: 'delegate'; agent: string; delegateTo: string }
  | { eventName: 'undelegate'; agent: string };
...
export type ThresholdEvent =
  | { eventName: 'propose'; agent: string; agentReputation: number; proposalId: string; title: string; description?: string; actionType: string; payload: unknown }
  | { eventName: 'vote'; agent: string; agentReputation: number; vote: VoteChoice }
  | { eventName: 'execute'; agent: string }
  | { eventName: 'reject'; agent: string }
  | { eventName: 'join'; agent: string; agentReputation: number }
  | { eventName: 'leave'; agent: string }
  | { eventName: 'propose_threshold_change'; agent: string; agentReputation: number; proposalId: string; memberThreshold?: number; voteThreshold?: number; proposeThreshold?: number };
 ...
 */
export function isSingleOwnerDAO(state: DAOState): state is SingleOwnerDAOState {
 ...
 */
export function isMultisigDAO(state: DAOState): state is MultisigDAOState {
 ...
 */
export function isTokenDAO(state: DAOState): state is TokenDAOState {
 ...
 */
export function isThresholdDAO(state: DAOState): state is ThresholdDAOState {
