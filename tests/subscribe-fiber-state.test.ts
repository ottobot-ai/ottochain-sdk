...
describe('MetagraphClient.subscribeFiberState', () => {
  ...
  describe('Group 3: Cleanup', () => {
    ...
    test('unsubscribe during active poll is safe', async () => {
      // Arrange
      const callback = jest.fn();
...
// Type exports test - these should be available when importing from the SDK
describe('Type Exports', () => {
  test('SubscribeOptions, FiberStateCallback, Unsubscribe types should be exported', () => {
    // This test will fail until the types are properly exported
    ...
    const callback: FiberStateCallback = (current: StateMachineFiberRecord | null, previous: StateMachineFiberRecord | null) => {
    ...
    // If we get here without TypeScript errors, the types are exported correctly
