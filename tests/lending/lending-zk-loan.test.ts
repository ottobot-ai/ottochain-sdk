...
describe("LendingZkLoan state machine", () => {
  ...
  it("drives the full REQUESTED→COLLATERAL_LOCKED→ACTIVE→REPAID happy path", () => {
    const byEvent = (e: string, from?: string) =>
      lendingZkLoanDef.transitions.find(
        (t) => t.eventName === e && (from ? t.from === from : true),
      );

    expect(byEvent("lock_collateral")).toMatchObject({
      from: "REQUESTED",
      to: "COLLATERAL_LOCKED",
    });
    expect(byEvent("originate")).toMatchObject({
  ...
  it("drives the default→liquidation path", () => {
    const def = lendingZkLoanDef.transitions.find(
      (t) => t.eventName === "default_loan",
    );
    const liq = lendingZkLoanDef.transitions.find(
      (t) => t.eventName === "liquidate",
    );
  ...
  it("every state carries non-null { label, description, category }", () => {
    ...
    expect(
      (lendingZkLoanDef.states.REQUESTED.metadata as Record<string, unknown>)
        .category,
    ).toBe("initial");
  ...
  describe("origination guard (the zk eligibility gate)", () => {
    const originate = lendingZkLoanDef.transitions.find(
      (t) => t.eventName === "originate",
    )!;
    ...
    const guardJson = originate.guard as unknown as {
    ...
    it("mints the debt asset to the borrower and activates on success", () => {
      ...
      expect(effectJson.merge[1]).toMatchObject({
        status: "ACTIVE",
  ...
  it("moves collateral via the reserved _transferAsset effect directive (not a dropped emits block)", () => {
    const repay = lendingZkLoanDef.transitions.find(
      (t) => t.eventName === "repay",
    )!;
    const liq = lendingZkLoanDef.transitions.find(
      (t) => t.eventName === "liquidate",
