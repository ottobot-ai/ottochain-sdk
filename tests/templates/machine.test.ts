...
const CONSUMER_DEF = JSON.parse(
  readFileSync(resolve(__dirname, '../fixtures/riverdale-economy/consumer.definition.json'), 'utf8'),
...
describe('templates/machine — machine() verified binding', () => {
  ...
  it('is pure/deterministic: same machine emits identical bodies across calls', () => {
    ...
    expect(m.create({ fiberId: 'fid', initialData: {} })).toEqual(m.create({ fiberId: 'fid', initialData: {} }));
  });
});
