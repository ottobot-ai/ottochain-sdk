...
import { transferAsset, toFiber } from "../../src/schema/effects";
...
describe("templates/machine — effect()", () => {
  ...
  it("is NOT merge-wrapped (flat map, no `merge` key)", () => {
    expect(effect({ status: "X" }, { _triggers: [] })).not.toHaveProperty(
      "merge",
    );
  });
    ...
    const built = transition({
      ...
      effect: effect(
        ...
        transferAsset([
          {
            assetId: { var: "event.payAssetId" },
            recipient: toFiber({ var: "event.retailerId" }),
          },
        ]),
      ),
    });
...
describe("templates/machine — machine() verified binding", () => {
  ...
  it("publish, create, upgrade, and wireDefinition share ONE definition object (===)", () => {
    ...
    expect(m.create({ fiberId: "fid", initialData: {} }).definition).toBe(
      m.wireDefinition(),
    );
    expect(
      m.upgradeFrom({ fiberId: "fid", targetSequenceNumber: 1 }).newDefinition,
    ).toBe(m.wireDefinition());
  });
  ...
  it("create references name@version via schemaRef and OMITS absent optionals", () => {
    ...
    expect(cre.schemaRef).toEqual({
      name: "consumer.package",
      version: { Exact: { version: "1.0.0" } },
    });
  ...
  it("upgradeFrom OMITS migration when absent, INCLUDES it (never null) when present", () => {
    ...
    expect(up2.migration).toEqual({
      merge: [{ var: "" }, { loyaltyPoints: 0 }],
    });
  });
