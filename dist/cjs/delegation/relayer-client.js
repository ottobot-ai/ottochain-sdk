...
class RelayerClient {
    constructor(config, relayerAddress, delegationManager) {
        if (!(0, utils_js_1.isValidAddress)(relayerAddress)) {
        ...
        this.delegationManager = delegationManager || new delegation_manager_js_1.DelegationManager(config);
    }
    /**
    ...
    async submitWithSessionKey(transaction, delegationId, sessionPrivateKey, gasConfig, signingFunction) {
        ...
        const hashedMessage = (0, utils_js_1.hashMessage)(transactionMessage);
        ...
        const sessionKeyProof = delegation_js_1.SessionKeyProof.create({
            sessionKey,
    ...
    async submitWithSignedIntent(delegationId, intentNonce, conditionProof, gasConfig) {
        ...
        const signedIntentProof = delegation_js_1.SignedIntentProof.create({
            signedIntent,
            conditionProof
        });
        // Create relayed transaction
    ...
    async createRelayedTransaction(options) {
        ...
        const hashedRelayerMessage = (0, utils_js_1.hashMessage)(relayerMessage);
        ...
        return delegation_js_1.RelayedTransaction.create(relayedTxData);
    }
    async submitRelayedTransaction(relayedTx) {
        const submitMessage = delegation_js_1.SubmitRelayedTransaction.create({ relayedTx });
    ...
    mergeGasConfig(gasConfig) {
        ...
        return delegation_js_1.GasConfig.create({
