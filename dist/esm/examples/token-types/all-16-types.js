 ...
 */
import { TokenBehaviorType, validateTokenOperation, getTERVFlags, TokenOperation, TokenState, } from '../../types/asset-model/token-types.js';
// ---------------------------------------------------------------------------
...
export const DigitalArtNFTExample = {
    ...
    transferArtwork: async (token, newOwner) => {
        ...
        const validation = validateTokenOperation(TokenBehaviorType.TERV_1001_AUTHENTICATED_COLLECTIBLE, TokenOperation.TRANSFER, context);
...
export const GameItemExample = {
    ...
    useItem: async (token, gameContext) => {
        ...
        const validation = validateTokenOperation(TokenBehaviorType.TERV_1101_CONSUMABLE_GAME_TOKEN, TokenOperation.CONSUME, context);
        if (!validation.allowed) {
...
export const DigitalCurrencyExample = {
    ...
    transfer: async (token, from, to, amount) => {
        ...
        const validation = validateTokenOperation(TokenBehaviorType.TERV_1111_FULL_DIGITAL_CURRENCY, TokenOperation.TRANSFER, context);
        if (!validation.allowed) {
...
export async function demonstrateTokenLifecycle() {
    ...
    try {
        ...
        const validation = validateTokenOperation(TokenBehaviorType.TERV_0000_BASIC_IDENTIFIER, TokenOperation.TRANSFER, context);
        if (validation.allowed) {
